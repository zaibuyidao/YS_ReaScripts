...
while integer ~= -1 do
    ...
    notedd = ppqend - startppqpos
    weizhi = startppqpos - ppqstart
...
reaper.ImGui_SetNextWindowSize(ctx, 410, 57)
...
function loop()
    ...
    if visible then
        ...
        if retval_esc or retval_enter then
            flag = false
        end
        editor = reaper.MIDIEditor_GetActive()
        if not editor then
